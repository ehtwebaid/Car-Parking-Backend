...
module.exports = {
  login: async function (req, resp) {
    ...
    try {
      ...
      if (!data.user?.email_verified_at) {
        return resp.status(200).json({
          status: "error",
          error_type: "email_not_validated",
  ...
  signup: async function (req, resp) {
    ...
    try {
      ...
      const otp = await helpers.createOtp({ user_id: newUser?.id, 'otp_type': 'E' });
      if (!otp) {
        return resp.status(200).json({
          status: "error",
      ...
      const send_email = await emailHelper.sendMail(req, {
        to: newUser?.email,
        subject: process.env.APP_NAME + ` - Confirm your login with code ${otp}`,
        type: "verify-email",
        data: {
